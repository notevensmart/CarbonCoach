...
export default function HeroSummaryCard({ dashboard }) {
  ...
  return (
    <section
      ...
      className="rounded-2xl bg-green-800 p-6 text-white shadow-lg"
    >
      ...
      {dashboard.total > 0 ? (
        ...
        <p className="mt-4 text-lg font-medium text-green-50">
      ...
      <dl className="mt-5 flex flex-wrap gap-x-8 gap-y-3 text-sm">
        {dashboard.mainDriver && (
          <div>
            <dt className="text-green-200">Main driver</dt>
            <dd className="font-semibold">{dashboard.mainDriver}</dd>
          </div>
        )}
        {confidence && (
          <div>
            <dt className="text-green-200">Confidence</dt>
